...
#define _S(x) #x
#define S(x) _S(x)
#define THROW_IF(x) do { if ((x)) { throw std::exception("projectM: assertion failed @" S(__LINE__) ## ":" ## S(x)); } } while(0)
#define THROW_IF_MSG(x, msg) do { if ((x)) { throw std::exception("projectM: " msg); } } while(0)
	void CProjectMWindow::initialize_gl() {
		...
		THROW_IF_MSG(!rc, "failed to create OpenGL context");
		THROW_IF(!dc.wglMakeCurrent(rc));
...
"Copyright (C) 2020 Andrey Kuleshov\n"
